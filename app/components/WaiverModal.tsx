          ...
          <ol className="space-y-3 text-sm text-white/60 leading-relaxed list-none">
            {[
              ...
              <li key={i} className="flex gap-2.5">
                <span className="text-[#FF2D94] font-black text-xs mt-0.5 flex-shrink-0">{i + 1}.</span>
                <span>{clause}</span>
              </li>
            ))}
          </ol>
          <p className="text-white/30 text-xs leading-relaxed mt-4 italic border-t border-white/8 pt-4">
        ...
        <div className="flex-shrink-0 px-5 pb-6 pt-4 border-t border-white/8 space-y-4 bg-[#12151D]">
          <label className="flex items-start gap-3 cursor-pointer">
            <input
              ...
              className="mt-0.5 w-5 h-5 accent-[#FF2D94] flex-shrink-0 cursor-pointer"
            />
            <span className="text-sm text-white/80 font-semibold leading-snug">
          ...
          <div className="flex gap-3">
            <button
              onClick={onCancel}
              className="flex-1 py-3 rounded-2xl border border-white/10 text-white/50 font-semibold text-sm hover:border-white/20 transition-colors">
              Cancel
            </button>
            <button
              onClick={onAccept}
              disabled={!ticked}
              className="flex-1 py-3 rounded-2xl bg-[#FF2D94] text-white font-black text-sm shadow-[0_0_16px_rgba(255,45,148,0.4)] disabled:opacity-40 disabled:cursor-not-allowed disabled:shadow-none transition-all">
              Join Race
            </button>
          </div>
        </div>
